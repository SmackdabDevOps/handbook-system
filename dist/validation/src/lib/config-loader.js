...
function findProjectRoot() {
    ...
    throw new Error('Could not find project root. No handbook.config.json or package.json found in current directory or any parent directory.');
}
/**
...
function validateConfig(config) {
    ...
    if (!validProfiles.includes(cfg.stackProfile)) {
        throw new Error(`Invalid stackProfile "${cfg.stackProfile}". Must be one of: ${validProfiles.join(', ')}`);
    }
    // Validate paths
    ...
    for (const pathKey of requiredPaths) {
        if (!(pathKey in cfg.paths) || typeof cfg.paths[pathKey] !== 'string') {
    ...
    for (const cmdKey of requiredCommands) {
        if (!(cmdKey in cfg.commands) || typeof cfg.commands[cmdKey] !== 'string') {
...
function loadConfig(projectRoot) {
    ...
    if (!fs.existsSync(configPath)) {
        throw new Error(`handbook.config.json not found at ${configPath}. ` +
    ...
    try {
        ...
        throw new Error(`Failed to read handbook.config.json at ${configPath}: ${error instanceof Error ? error.message : String(error)}`);
    }
    let config;
    try {
        ...
        throw new Error(`Failed to parse handbook.config.json at ${configPath}: ${error instanceof Error ? error.message : String(error)}`);
    }
    try {
        validateConfig(config);
    }
    catch (error) {
        throw new Error(`Invalid handbook.config.json at ${configPath}: ${error instanceof Error ? error.message : String(error)}`);
    }
    return config;
}
/**
 ...
 */
function resolvePath(config, configPath) {
