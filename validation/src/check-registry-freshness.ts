...
import { loadConfig, findProjectRoot, getValidatorsPath, getRegistryPath } from './config/loader';
 ...
 */
function discoverValidators(config: any, validatorsPath: string): string[] {
 ...
 */
function getCurrentGitCommit(projectRoot: string): string | null {
  try {
 ...
 */
function getCommitsBetween(from: string, to: string, projectRoot: string): number {
  try {
    const output = execSync(`git rev-list ${from}..${to} --count`, {
      cwd: projectRoot,
      encoding: 'utf-8',
    }).trim();
...
function checkFreshness(
  ...
  for (const gate of Object.values(registry.phaseGates || {})) {
    for (const v of (gate as any).requiredValidators || []) {
...
function main() {
  try {
    ...
    console.log(
      `  Last full run: ${report.registryAge !== null ? formatDuration(report.registryAge) + ' ago' : 'Never'}`,
    );
    console.log('');
