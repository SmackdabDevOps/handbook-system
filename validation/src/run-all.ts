...
import { loadConfig, findProjectRoot, getAbsolutePath, getValidatorsPath } from './config/loader';

// Import lib functions if available
let runSuite: any;
let reportValidationResult: any;
let markFullRunComplete: any;
...
try {
  ...
  runSuite = async (cmds: any[], executor: any) => {
...
function runBlocking(title: string, cmd: string[], cwd: string) {
  console.log(`>>> ${title}: ${cmd.join(' ')}`);
  const res = spawnSync(cmd[0], cmd.slice(1), { stdio: 'inherit', cwd });
  if (res.status !== 0) {
    throw new Error(`${title} failed with exit code ${res.status ?? 1}`);
  }
}

function getRequestedValidator(): string | null {
 ...
 */
function discoverValidators(config: any, validatorsPath: string): SuiteCommand[] {
  const validators: SuiteCommand[] = [];
...
async function runValidators(
  ...
  const validatorsPath = getValidatorsPath(projectRoot);
  ...
  const result = await runSuite(suite, async (cmd: any) => {
...
async function main() {
  ...
  try {
    ...
    const progressScript = path.join(getValidatorsPath(projectRoot), 'generate-progress-files.ts');
    if (fs.existsSync(progressScript)) {
      spawnSync('npx', ['ts-node', progressScript], { cwd: projectRoot, stdio: 'inherit' });
    }

    spawnSync('npm', ['run', 'dashboard:update'], { cwd: projectRoot, stdio: 'inherit' });
